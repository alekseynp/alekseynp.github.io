...
var chartCore = vis.append("svg")
 ...
 d3.csv("projects/olympics2012/data.csv",function(csvDetail) {
   drawData(csvDetail);
 });
...
vis.append("text")
  .text("South")
  .attr("x", -8)
  .attr("y", chartHeight-chartTopPadding-chartBottomPadding)
  .style("text-anchor", "end")
  .style("font-size", "12px")
  .style("fill", "#aaa");  
...
// Stepper data
var stepperTitles = ["Introduction", "Participants vs. Medals", "Table Tennis Paricipants", "Table Tennis Medals"];
var stepperText = ["<p>The chart is displaying the average longitude/latitude of medals won from the 2012 London Summer Olympics. Larger bubbles are for sports with more medals.</p><p>Use this chart to learn the geography of sports. Who comes to compete, and who wins?</p>", "<p>Now the chart is showing paricipants. The sports are tightly clustered because there are a broad range of participants from all over the world.</p><p>Football and Hockey are relatively southern due to low participation from Europe.<p>Larger bubbles have more participants.</a>", "<p>Table Tennis is an east-of-centre sport with participants from all over including many Europeans.</p>", "<p>Switching to medals, however, we see that the sport is actually dominated by the East.</p><p>Note that badminton and archery follow similar trends.</p>"];
